...
function start_detect()
    ...
    if not mp.command(cmd) then
        ...
        mp.osd_message("autodeint: failed to insert detection filter")
        return
    end

    -- wait to gather data
    mp.osd_message("autodeint: starting detection (field dominance)")
...
function judge_field_dominance()
    ...
    if not mp.command(cmd) then
        ...
        mp.osd_message("autodeint: failed to insert pullup filter")
        stop_detect()
        return
    end
    ...
    mp.osd_message("autodeint: starting detection (ivtc)")
